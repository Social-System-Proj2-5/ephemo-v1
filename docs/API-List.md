...
- REST API
- JSON
- ファイル送信は `multipart/form-data`
- 認証は `Authorization: Bearer <token>`
- フロントからDBへ直接アクセスしない
- 交換対象は `Asset` ではなく `Ephemera`
...
|  No | 機能             | Method | URL                         | フロントから送るもの    | バックエンドが返すもの                  | DBとの関わり                    |
| --: | ---------------- | ------ | --------------------------- | ----------------------- | --------------------------------------- | ------------------------------- |
|   6 | 素材一覧取得     | GET    | `/api/v1/assets`            | `type` 任意             | `assets[]`                              | `assets` から自分の素材だけ取得 |
|   7 | 素材詳細取得     | GET    | `/api/v1/assets/{asset_id}` | `asset_id`              | 素材詳細                                | `assets` を取得                 |
...
|  No | 機能               | Method | URL                               | フロントから送るもの                                  | バックエンドが返すもの | DBとの関わり                     |
| --: | ------------------ | ------ | --------------------------------- | ----------------------------------------------------- | ---------------------- | -------------------------------- |
|  16 | エフェメラ一覧取得 | GET    | `/api/v1/ephemeras`               | `is_exchangeable` 任意                                | `ephemeras[]`          | `ephemeras` から自分のものを取得 |
|  17 | エフェメラ詳細取得 | GET    | `/api/v1/ephemeras/{ephemera_id}` | `ephemera_id`                                         | エフェメラ詳細         | `ephemeras` を取得               |
|  18 | エフェメラ作成     | POST   | `/api/v1/ephemeras`               | `asset_id`, `template_id`, `title`, `is_exchangeable` | `ephemera_id`          | `ephemeras` に追加               |
|  19 | エフェメラ更新     | PATCH  | `/api/v1/ephemeras/{ephemera_id}` | `title`, `is_exchangeable`                            | 更新後の情報           | `ephemeras` を更新               |
...
|  26 | アイテム追加             | POST   | `/api/v1/scrapbooks/{scrapbook_id}/items`           | `ephemera_id`, `x`, `y`, `width`, `height` | `scrapbook_item_id`            | `scrapbook_items` に追加               |
|  27 | アイテム配置更新         | PATCH  | `/api/v1/scrapbooks/{scrapbook_id}/items/{item_id}` | `x`, `y`, `width`, `height`, `rotation`    | 更新後の配置                   | `scrapbook_items` を更新               |
...
|  No | 機能                           | Method | URL                                              | フロントから送るもの | バックエンドが返すもの                            | DBとの関わり                                            |
| --: | ------------------------------ | ------ | ------------------------------------------------ | -------------------- | ------------------------------------------------- | ------------------------------------------------------- |
|  29 | 交換セッション作成             | POST   | `/api/v1/exchange/sessions`                      | `my_ephemera_id`     | `exchange_session_id`, `qr_payload`, `expires_at` | `exchange_sessions` に追加                              |
|  30 | 交換セッション確認             | GET    | `/api/v1/exchange/sessions/{session_id}`         | `session_id`         | 相手の表示名、交換対象プレビュー                  | `exchange_sessions`, `users`, `ephemeras` を取得        |
|  31 | 交換に出す自分のエフェメラ選択 | PATCH  | `/api/v1/exchange/sessions/{session_id}/offer`   | `my_ephemera_id`     | 選択結果                                          | `exchange_sessions` を更新                              |
|  32 | 交換確定                       | POST   | `/api/v1/exchange/sessions/{session_id}/confirm` | `confirm: true`      | 交換成立結果                                      | `exchange_records`, `ephemeras`, `point_records` に追加 |
...
|  35 | 交換履歴詳細取得 | GET    | `/api/v1/exchanges/history/{record_id}` | `record_id`            | 交換詳細               | `exchange_records`, `ephemeras`, `users` を取得 |
...
2. セッションの有効期限を確認する
3. 自分自身との交換でないか確認する
4. 双方の `ephemera_id` が存在するか確認する
5. `ephemeras.is_exchangeable` が `true` か確認する
6. AさんのエフェメラをBさん側へコピーする
7. BさんのエフェメラをAさん側へコピーする
8. `exchange_records` に交換履歴を追加する
9. `point_records` にポイント付与履歴を追加する
10. `exchange_sessions` を使用済みに更新する
